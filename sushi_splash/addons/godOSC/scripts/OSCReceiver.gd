...
func received_message(address, vals, time):
	...
	if address !="/packet":
		return
	#print("/packet values: ", incoming_values)
	
	#var norm = Vector2(float(value[0]), float(value[1]))
	var norm = Vector2(
			float(remap(float(vals[0]),-1,1,remapX.x,remapX.y)),
			float(remap(float(vals[1]),-1,1,remapY.x,remapY.y))
			)
	var latest_packet = norm * screen_half + screen_center * Vector2(1,-1)+Vector2(0,1080)
	#$PlayerPredictionSystem.new_position_recieved(latest_packet)
	
	
	
	get_parent().new_position_recieved(latest_packet)
	pass
